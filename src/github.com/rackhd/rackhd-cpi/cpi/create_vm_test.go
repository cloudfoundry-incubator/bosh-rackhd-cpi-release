  ...
  Describe("randomSelectAvailableNode", func() {
    Context("when all nodes are reserved", func() {
      It("returns an error", func() {
        ...
        server.RouteToHandler("GET", fmt.Sprintf("/api/2.0/nodes/%s/workflows?active=true", nodes[0].ID),
          ghttp.CombineHandlers(
            ghttp.VerifyRequest("GET", fmt.Sprintf("/api/2.0/nodes/%s/workflows?active=true", nodes[0].ID)),
            ghttp.RespondWith(http.StatusNoContent, []byte{}),
          ),
        )

        server.RouteToHandler("GET", fmt.Sprintf("/api/2.0/nodes/%s", nodes[0].ID),
          ghttp.CombineHandlers(
            ghttp.VerifyRequest("GET", fmt.Sprintf("/api/2.0/nodes/%s", nodes[0].ID)),
            ghttp.RespondWith(http.StatusOK, node0HttpResponse),
          ),
        )

        server.RouteToHandler("GET", fmt.Sprintf("/api/2.0/nodes/%s/workflows?active=true", nodes[1].ID),
          ghttp.CombineHandlers(
            ghttp.VerifyRequest("GET", fmt.Sprintf("/api/2.0/nodes/%s/workflows?active=true", nodes[1].ID)),
            ghttp.RespondWith(http.StatusNoContent, []byte{}),
          ),
        )

        server.RouteToHandler("GET", fmt.Sprintf("/api/2.0/nodes/%s", nodes[1].ID),
          ghttp.CombineHandlers(
            ghttp.VerifyRequest("GET", fmt.Sprintf("/api/2.0/nodes/%s", nodes[1].ID)),
    ...
    Context("when all nodes are provisioned", func() {
    ...
    Context("when a node does not have obmsettings", func() {
      It("skips the node", func() {
        rawWorkflow := helpers.LoadJSON("../spec_assets/dummy_workflow_response.json")
        httpWorkflowsResponse := []byte(fmt.Sprintf("[%s]", string(rawWorkflow)))
        var expectedResponse []models.WorkflowResponse
    ...
    Context("when there is an available node", func() {
      It("selects a free node for provisioning", func() {
        ...
        server.RouteToHandler("GET", fmt.Sprintf("/api/2.0/nodes/%s/workflows", nodes[0].ID),
          ghttp.CombineHandlers(
            ghttp.VerifyRequest("GET", fmt.Sprintf("/api/2.0/nodes/%s/workflows", nodes[0].ID), "active=true"),
            ghttp.RespondWith(http.StatusNoContent, []byte{}),
          ),
        )
  ...
  Describe("retrying node reservation", func() {
    It("return a node if selection is successful", func() {
      ...
      nodeID, err := TryReservation(
        ...
        func(config.Cpi, models.Node) error { return nil },
      )
      Expect(err).ToNot(HaveOccurred())
    ...
    It("returns an error if selection continually fails", func() {
      ...
      nodeID, err := TryReservation(
        ...
        "",
        func(config.Cpi, string, Filter) (models.Node, error) { return models.Node{ID: "node-1234"}, nil },
        func(config.Cpi, models.Node) error { return errors.New("error") },
      )
    ...
    It("cleans up reservation flag after receive timeout error on reserve function", func() {
      ...
      flakeyReservationFunc := func(c config.Cpi, node models.Node) error {
        testNodeID = node.ID
        url := fmt.Sprintf("%s/api/2.0/nodes/%s", c.ApiServer, node.ID)
        ...
        nodeURL := fmt.Sprintf("%s/api/2.0/nodes/%s", c.ApiServer, testNodeID)
        ...
        Expect(err).ToNot(HaveOccurred())
        Expect(expectedNode.Status).To(Equal(models.Reserved))
