    ...
    Context("with enough disk space", func() {
      Context("If VM cid is empty", func() {
        It("creates the disk and returns the disk cid", func() {
          ...
          nodeID := "57fb9fb03fcc55c807add402"
          ...
          server.AppendHandlers(
            ...
            ghttp.CombineHandlers(
              ghttp.VerifyRequest("PATCH", fmt.Sprintf("/api/2.0/nodes/%s/tags", nodeID)),
      ...
      Context("If VM CID is not empty", func() {
        It("creates the disk and returns the disk cid", func() {
          ...
          server.AppendHandlers(
            ...
            ghttp.CombineHandlers(
              ghttp.VerifyRequest("PATCH", fmt.Sprintf("/api/2.0/nodes/%s/tags", expectedNodes[0].ID)),
              ghttp.RespondWith(http.StatusOK, nil),
            ),
          )
